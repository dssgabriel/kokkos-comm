...
#include "utils.hpp"
#ifdef KOKKOSCOMM_ENABLE_NCCL
#include "../unit_tests/nccl/utils.hpp"  // For test_utils::nccl::Ctx
#endif
...
void osu_lat_KC(
    benchmark::State&, Comm& comm, const View& sv, const View& rv
) {
  if (comm.rank() == 0) {
...
void osu_lat_KC_mpi(
    benchmark::State&, KC::Communicator<KC::MpiSpace, DES>& comm, const View& sv, const View& rv
) {
  if (comm.rank() == 0) {
    KC::mpi::send(comm, sv, 1, 0);
    KC::mpi::recv(comm, rv, 1, 1);
  } else {
    KC::mpi::recv(comm, rv, 0, 0);
    KC::mpi::send(comm, sv, 0, 1);
  }
}
...
void osu_lat_KC_nccl(benchmark::State&, KC::Communicator<KCE::NcclSpace, DES>& comm, const View& sv, const View& rv) {
  if (comm.rank() == 0) {
    auto s_req = KCE::nccl::send(comm, sv, 1);
    auto r_req = KCE::nccl::recv(comm, rv, 1);
    s_req.wait();
    r_req.wait();
  } else {
    auto r_req = KCE::nccl::recv(comm, rv, 0);
    auto s_req = KCE::nccl::send(comm, sv, 0);
    r_req.wait();
    s_req.wait();
  }
}
...
auto bench_KC_NcclSpace(benchmark::State& state) -> void {
  auto& nccl_ctx = test_utils::nccl::Ctx::get();
